...
def slide_data(data, rank, world_size, order=3, processed=True):
    ...
    queue.put(kwargs)
    t = Slide_data_thread(rank, queue)
    t.start()
    t.join()

    t_features, t_features_dict, t_targets, t_targets_dic = t.q.get()

    return t_features_dict, t_targets, t_targets_dic, q_ids
...
def file_combine(path, word_size, type='pkl'):
    import pickle
    out_dict = {}
    for i in range(word_size):
        print("begin to load file: ", path+'ws_'+str(word_size)+'rank_'+str(i))
        with open(path+'ws_'+str(word_size)+'rank_'+str(i), 'rb') as f:
            data_dic = pickle.load(f)
            out_dict = {**out_dict, **data_dic}
        print("finish reading file: ", path + 'ws_' + str(word_size) + 'rank_' + str(i))
    data2pickle(out_dict, path)
...
if __name__=='__main__':
    ...
    features_dict, targets, targets_dic, q_ids = slide_data(data, rank, word_size, order=2)
    data2pickle(features_dict, './features.pkl'+'ws_'+str(word_size)+'rank_'+str(rank))
    data2pickle(targets_dic, './targets.pkl'+'ws_'+str(word_size)+'rank_'+str(rank))
    # data2csv(targets, './targets.csv'+'ws_'+str(word_size)+'rank_'+str(rank))

    sync = torch.zeros(1)
    dist.all_reduce(sync, async_op=False)
    if dist.get_rank() == 0:
        file_combine('./features.pkl', word_size, type='pkl')
        file_combine('./targets.pkl', word_size, type='pkl')
